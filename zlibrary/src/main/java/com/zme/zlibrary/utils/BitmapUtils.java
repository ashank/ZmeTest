...
public class BitmapUtils {
     ...
     * @param context
     * @param resId  图片在本地的资源ID
     * @return 返回  图片编码为 Config.RGB_565的Bitmap对象
     */
    ...
    public static byte[] compressBmp(String filePath) {
        ...
        if (null != bmp) {
            ...
            LogUtils.e("lenght>>>>" + baos.toByteArray().length / 1024);
            bmp.recycle();
            bmp = null;
        }
     ...
     */
    private static Bitmap rotateImage(Bitmap bmp, int degrees) {
        if (degrees != 0) {
            Matrix matrix = new Matrix();
     ...
     * 从SD卡中读取到图片,进行压缩
     * @param filePath
     * @param compressImageMaxSize  希望压缩后的图片最大的大小值
     * @return  返回压缩后的Bitmap
     */
    public static Bitmap compressBitmap(@NonNull String filePath,@NonNull int compressImageMaxSize) {
        Bitmap bmp = null;
        ByteArrayOutputStream baos = null;
        //将图片转为Bitmap
        BitmapFactory.Options newOpts = new BitmapFactory.Options();
        ...
        if (null != bmp) {
            ...
            bmp.compress(Bitmap.CompressFormat.JPEG, options, baos);
            //如果压缩的结果大于700k,持续压缩
            while (baos.toByteArray().length / 1024 > compressImageMaxSize && options > 0) {
                baos.reset();
                options -= 5;
                if (options > 0) {
                    bmp.compress(Bitmap.CompressFormat.JPEG, options, baos);
